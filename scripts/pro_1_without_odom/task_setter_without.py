...
def manager(data):
	...
	if(data.z == 0):
		...
		calcToPoint(data) # jesli po wykonaniu tej funkcji working==True, tzn ze przyjal te robote
		if(working):
			print "ZACZYNAM RUCH DO PKT I Z POWROTEM"
			talkerToSlide()
			working = False
			STATE = -1
		...
		calcToPoint(data) # jesli po wykonaniu tej funkcji working==True, tzn ze przyjal te robote
		if(working):
			print "ZACZYNAM RUCH KWADRAT"
			talkerToPoint()
			cuRoPo.x = hopeRoPo.x
			cuRoPo.y = hopeRoPo.y
			cuRoPo.theta = hopeRoPo.theta
			[msg1,msg2,msg3] = calcAllSquarePoints()
			pose = Pose()
			pose.position.x = msg1.x
	...
	"""	
		...
		if(STATE == 3): return # nie rob kwadratu drugi raz z rzedu
		STATE = 3
		calcToPoint(data) # jesli po wykonaniu tej funkcji working==True, tzn ze przyjal te robote
		if(working):
...
def talkerToSlide():
	...
	message = buildMess(True, False, -1)
	i=0
	while i <= HZ*setRoPo.tGo:
		pub.publish(message)
		i=i+1
